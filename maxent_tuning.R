...
bg <- as.data.frame(bg) # ljx added to change bg from matrix to dataframe
...
setwd("D:/工作/2013 PFS-Tropical Asia/PFS_Fixed_AFEC-X/AFEC-X 2021/PPTs etc/1111-12 QiaoHJ_SDM/enm_training/result")
saveRDS(bg, "example_bg.rda")
saveRDS(maxent_model, "example_maxent_model.rda")

r <- predict(maxent_model, predictors)
writeRaster(r, "example_maxent_result.tif")
plot(r)
